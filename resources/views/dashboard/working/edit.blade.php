...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Penugasan</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <form class="row g-3 p-2" action="{{ route('dashboard.working.update', $working->id) }}" method="POST"
                ...
                <div class="col-6 mb-3">
                    ...
                    <input type="number" class="form-control @error('code') is-invalid @enderror" id="code"
                        name="code" value="{{ old('code', $working->code) }}" required>
                    @error('code')
                ...
                <div class="col-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('hours') is-invalid @enderror" id="hours"
                        name="hours" value="{{ old('hours', $working->hours) }}" required>
                    @error('hours')
                        <span class="text-danger text-sm" role="alert">
                            {{ $message }}
                        </span>
                    ...
                    <select class="form-select  @error('rombel') is-invalid @enderror" name="rombel" id="select2"
                        ...
                        @foreach ($rombels as $rombel)
                            <option value="{{ $rombel['id'] }}"
                                {{ old('rombel', $working->grade_id . ' ' . $working->group_id) == $rombel['id'] ? ' selected' : ' ' }}>
                                {{ $rombel['name'] }}</option>
                        @endforeach
                    </select>
                    @error('rombel')
                    ...
                    <select class="form-select  @error('teacher_id') is-invalid @enderror" name="teacher_id" id="select2"
                        ...
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->id }}"
                                {{ old('teacher_id', $working->teacher_id) == $teacher->id ? ' selected' : ' ' }}>
                                {{ $teacher->name }}</option>
                        @endforeach
                    </select>
                    @error('teacher_id')
                    ...
                    <select class="form-select  @error('subject_id') is-invalid @enderror" name="subject_id" id="select2"
                        ...
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}"
                                {{ old('subject_id', $working->subject_id) == $subject->id ? ' selected' : ' ' }}>
                                {{ $subject->name }}</option>
                        @endforeach
                    </select>
                    @error('subject_id')
